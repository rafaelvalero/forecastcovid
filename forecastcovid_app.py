...
from covid_forecast.utils.data_io import get_data, download_csv_from_link,get_data_latest_folder
from bokeh.io import output_file, show
...
#Put date is a good format to sort
data['dateRep'] = pd.to_datetime(data['dateRep'], format='%d/%m/%Y')
"""Some documentation"""
homepage = Div(text=open(os.path.join(os.getcwd(),resources_folder+ '/homepage.html')).read(), width=800)
explanation_sir_model = Div(text=open(os.path.join(os.getcwd(), resources_folder+ '/explanation_model.html')).read(), width=800)
""" Key functions """
